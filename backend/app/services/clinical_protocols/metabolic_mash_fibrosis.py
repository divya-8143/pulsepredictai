"""
MASLD/MASH Non-Invasive Surveillance & Resmetirom Pathway
...
@dataclass
class MetabolicMashFibrosisContext:
    patient_id: str
    age: float
    gender: str
...
class MetabolicMashFibrosisProtocolEngine:
    """
    Standardized Protocol Implementation for MASLD/MASH Non-Invasive Surveillance & Resmetirom Pathway.
    """

    PROTOCOL_NAME = "MASLD/MASH Non-Invasive Surveillance & Resmetirom Pathway"
    SPECIALTY = "Hepatology"

    @classmethod
    def execute_protocol(cls, ctx: MetabolicMashFibrosisContext) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def _determine_step(cls, ctx: MetabolicMashFibrosisContext) -> int:
    ...
    @classmethod
    def _generate_dosing_plan(cls, step: int, ctx: MetabolicMashFibrosisContext) -> List[Dict[str, str]]:
        if step == 3:
            return [
    ...
    @classmethod
    def _generate_monitoring_schedule(cls, step: int, ctx: MetabolicMashFibrosisContext) -> List[Dict[str, str]]:
        return [
    ...
    @classmethod
    def _perform_safety_checks(cls, ctx: MetabolicMashFibrosisContext) -> List[str]:
        alerts = []
