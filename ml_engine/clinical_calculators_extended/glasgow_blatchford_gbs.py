"""
Glasgow-Blatchford Bleeding Score (GBS) for Upper GI Bleeding
Clinical Reference: Blatchford et al. Lancet 2000
...
@dataclass
class GlasgowBlatchfordGbsParameters:
...
class GlasgowBlatchfordGbsEngine:
    """
    Precision algorithmic evaluator for Glasgow-Blatchford Bleeding Score (GBS) for Upper GI Bleeding.
    Reference: Blatchford et al. Lancet 2000
    """

    ALGORITHM_NAME = "Glasgow-Blatchford Bleeding Score (GBS) for Upper GI Bleeding"
    CITATION = "Blatchford et al. Lancet 2000"
    SPECIALTY = "Gastroenterology"

    @classmethod
    def evaluate(cls, params: GlasgowBlatchfordGbsParameters) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def _compute_raw_score(cls, params: GlasgowBlatchfordGbsParameters) -> float:
    ...
    @classmethod
    def _derive_probability(cls, raw: float, params: GlasgowBlatchfordGbsParameters) -> float:
    ...
    @classmethod
    def _generate_recommendations(cls, tier: str, params: GlasgowBlatchfordGbsParameters) -> List[Dict[str, str]]:
        recs = []
        if tier == "HIGH_RISK_INTENSIVE":
            recs.append({
                "category": "Immediate Action",
                "directive": f"High risk identified by Glasgow-Blatchford Bleeding Score (GBS) for Upper GI Bleeding. Urgent clinical review and telemetry monitoring recommended.",
