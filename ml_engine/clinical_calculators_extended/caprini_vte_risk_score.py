"""
Caprini Score for Venous Thromboembolism in Surgical Inpatients
Clinical Reference: Caprini et al. Dis Mon 2005
...
@dataclass
class CapriniVteRiskScoreParameters:
...
class CapriniVteRiskScoreEngine:
    """
    Precision algorithmic evaluator for Caprini Score for Venous Thromboembolism in Surgical Inpatients.
    Reference: Caprini et al. Dis Mon 2005
    """

    ALGORITHM_NAME = "Caprini Score for Venous Thromboembolism in Surgical Inpatients"
    CITATION = "Caprini et al. Dis Mon 2005"
    SPECIALTY = "Vascular Surgery"

    @classmethod
    def evaluate(cls, params: CapriniVteRiskScoreParameters) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def _compute_raw_score(cls, params: CapriniVteRiskScoreParameters) -> float:
    ...
    @classmethod
    def _derive_probability(cls, raw: float, params: CapriniVteRiskScoreParameters) -> float:
    ...
    @classmethod
    def _generate_recommendations(cls, tier: str, params: CapriniVteRiskScoreParameters) -> List[Dict[str, str]]:
        recs = []
        if tier == "HIGH_RISK_INTENSIVE":
            recs.append({
                "category": "Immediate Action",
                "directive": f"High risk identified by Caprini Score for Venous Thromboembolism in Surgical Inpatients. Urgent clinical review and telemetry monitoring recommended.",
